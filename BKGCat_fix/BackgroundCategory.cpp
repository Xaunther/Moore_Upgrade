...
//=============================================================================
BackgroundCategory::BackgroundCategory( const std::string& type, const std::string& name, const IInterface* parent )
  ...
  declareProperty( "Calo2MCWeight", m_caloWeight = 0.5 ); ///< equivalent of m_override for neutral calorimetric objects
...
//=============================================================================
IBackgroundCategory::Result BackgroundCategory::category( const LHCb::Particle* reconstructed_mother,
                                                          const LHCb::Particle* )
...
// all the categories mean, please visit IBackgroundCategory.h
{
  ...
  if ( !reconstructed_mother ) { Exception( "Given nothing to classify. Bye!" ); }

  // Initialize the value of the common mother to zero
  m_commonMother = NULL;

  if ( !m_smartAssociator ) { Exception( "Something failed when making the associators. Bye!" ); }

  if ( !m_particleDescendants ) { Exception( "Something failed when making the particle descendents tool. Bye!" ); }

  // Some debug information at VERBOSE level
  if ( msgLevel( MSG::VERBOSE ) )
    verbose() << "About to start processing the categorisation tree for a " << reconstructed_mother->particleID().pid()
              << " " << reconstructed_mother->momentum() << endmsg;

  if ( m_printDecay.isEnabled() ) m_printDecay->printTree( reconstructed_mother );
  ...
  if ( doAllFinalStateParticlesHaveACommonMother( mc_mothers_final, mc_particles_linked_to_decay,
                                                  particles_in_decay ) ) {
...
// of 'hidden' method.
{
  if ( !reconstructed_mother ) { Exception( "Got NULL pointer" ); }

  const int backcategory = category( reconstructed_mother ).category;
  ...
  for ( DaughterAndPartnerVector::const_iterator iP = m_daughtersAndPartners.begin();
    ...
    if ( ( 0 != ( *iP ).second ) && ( msgLevel( MSG::VERBOSE ) ) )
      verbose() << "Associated particle has PID" << ( ( *iP ).second )->particleID().pid() << endmsg;
    else if ( msgLevel( MSG::VERBOSE ) )
      verbose() << "There is no associated particle" << endmsg;
  }
...
// PID as your composite particle!
{
  ...
  int backcategory                = IBackgroundCategory::Undefined; // don't reclassify if category is already known
  IBackgroundCategory::Result res = m_bkg->category( reconstructed_mother );
  if ( res.quality != IBackgroundCategory::Good ) { // if category cannot be obtained from Relation Tables
    if ( msgLevel( MSG::VERBOSE ) ) verbose() << "Getting BKGCAT and the common mother" << endmsg;
    backcategory = category( reconstructed_mother ).category; // try to recover info, not completely safe on uDST
    if ( backcategory >= IBackgroundCategory::Ghost ) {
      if ( msgLevel( MSG::VERBOSE ) ) verbose() << "No common mother for BackgroundCategory >= 60" << endmsg;
      return nullptr; // no mother for cat >= 60
    }
    return m_commonMother;
  } else { // run only needed methods to get common mother
    backcategory = res.category;
    if ( backcategory >= IBackgroundCategory::Ghost ) {
      if ( msgLevel( MSG::VERBOSE ) ) verbose() << "No common mother for BackgroundCategory >= 60" << endmsg;
      return nullptr; // no mother for cat >= 60
    }
    ...
    // We use Patrick's tool to get all the particles in the decay tree.
    ParticleVector particles_in_decay = m_particleDescendants->descendants( reconstructed_mother );
    if ( particles_in_decay.empty() ) { return nullptr; }
    // Now we have to associate each one of them to an MCParticle if possible
    const MCParticleVector mc_particles_linked_to_decay =
        associate_particles_in_decay( particles_in_decay, reconstructed_mother );
    // Now to create a vector with the final mothers of all these mc particles.
    MCParticleVector mc_mothers_final = get_mc_mothers( mc_particles_linked_to_decay );
    doAllFinalStateParticlesHaveACommonMother( mc_mothers_final, mc_particles_linked_to_decay, particles_in_decay );
    return m_commonMother;
  }
}
//=============================================================================
...
// this tool. Not the cleverest way to do this, but works OK.
{
  ...
  if ( pid == 11 ||                            // electron
       ...
       pid == 13 ||                            // muon
       pid == 12 ||                            // neutrinos
       pid == 14 || pid == 16 || pid == 211 || // pion (charged)
...
// except photons and neutrinos
{
  ...
  for ( iV = motherEndVertices.begin(); iV != motherEndVertices.end(); ++iV ) {
    if ( *iV ) {
      if ( ( *iV )->isDecay() ) { break; }
    } // Shouldn't happen but if some empty vertex sneaks into the list, just skip it...
  }
  if ( iV == motherEndVertices.end() )
    // Revert to the first vertex in the list
    iV = motherEndVertices.begin();
  ...
  for ( SmartRefVector<LHCb::MCParticle>::const_iterator iP = ( *iV )->products().begin();
    ...
    if ( m_useSoftPhotonCut && ( *iP )->particleID().abspid() == 22 && ( *iP )->momentum().e() < m_softPhotonCut )
      continue; // soft photons are ignored
    ...
    // Print out some debug messages
    if ( msgLevel( MSG::VERBOSE ) )
      verbose() << "Printing out decay vertices of a " << ( *iP )->particleID().abspid()
                << " (E=" << ( *iP )->momentum().e() << ") " << endmsg;
    ...
    if ( isitstable ||                              // if the MCParticle did not have an endVertex
      ...
      if ( ( ( *iP )->particleID().abspid() != 12 ) && ( ( *iP )->particleID().abspid() != 14 ) &&
      ...
      if ( msgLevel( MSG::VERBOSE ) )
        verbose() << "MC sum having added a charged or composite is now " << sumofpids << endmsg;
    }
  }
...
// that was originally passed to The Cat, except photons (we never reconstruct neutrinos anyway)
{
  ...
  for ( ParticleVector::const_iterator iP = particles_in_decay.begin(); iP != particles_in_decay.end(); ++iP ) {
...
{
  if ( msgLevel( MSG::VERBOSE ) ) verbose() << "Creating an array of final state daughters for the mc mother" << endmsg;
  ...
  if ( msgLevel( MSG::VERBOSE ) )
    verbose() << "Mother " << topmother->particleID().pid() << " has " << ( *iV )->products().size() << " daughters"
              << endmsg;
  ...
  for ( SmartRefVector<LHCb::MCParticle>::const_iterator iP = ( *iV )->products().begin();
    ...
    if ( msgLevel( MSG::VERBOSE ) ) {
      ...
      if ( isitstable )
        ...
        verbose() << " products: " << ( *( VV.begin() ) )->products().size() << endmsg;
    }
    ...
    if ( isitstable || isStable( ( *iP )->particleID().abspid() ) ) {
      ...
      finalstateproducts.insert( finalstateproducts.end(), tempfinalstateproducts.begin(),
...
// Gets the 'final' non-zero mother of an MCParticle as we go up the decay tree
{
  ...
  do {
    ...
    if ( m_ignoreQuarks && tmpmother != NULL ) {
      if ( tmpmother->particleID().isQuark() ) { return finalmother; }
    }
...
//=============================================================================
const LHCb::MCParticle* BackgroundCategory::get_lowest_common_mother( const MCParticleVector& mc_particles_to_compare )
...
// then take the second mother of the first particle, check... etc.
{
  ...
  if ( msgLevel( MSG::VERBOSE ) )
    verbose() << "The tempmother has been set to " << tempmother << " with PID " << tempmother->particleID().pid()
              << endmsg;
  ...
  do {
    ...
    if ( !tempmother ) return nullptr;
    // Exception( "Something went wrong in the MCParticle tree, please report the bug." ); // shouldn't happen!

    if ( msgLevel( MSG::VERBOSE ) )
      verbose() << "The current candidate for a common mother is " << tempmother << " and has PID "
                << tempmother->particleID().pid() << endmsg;

    for ( iMCP = mc_particles_to_compare.begin() + 1; iMCP != mc_particles_to_compare.end(); ++iMCP ) {
      ...
      do {
        if ( tempmother2 != tempmother )
          tempmother2 = tempmother2->mother();
        else
          carryon = false;
        if ( tempmother2 != NULL ) {
          if ( msgLevel( MSG::VERBOSE ) )
            verbose() << "Not found the original mother yet, currently on " << tempmother2 << " with PID "
                      << tempmother2->particleID().pid() << endmsg;
        }
  ...
  do {
    if ( *iPP ) {
      ...
      if ( isStable( ( *iPP )->particleID().abspid() ) || ( *iPP )->isBasicParticle() ) {
        ...
        if ( msgLevel( MSG::VERBOSE ) )
          verbose() << "Pushed back MCParticle " << ( *iMCP ) << " with PID " << ( *iMCP )->particleID().abspid()
                    << endmsg;
      }
      ++iMCP;
      ++iPP;
      ...
      return nullptr;
    // Exception( "Your reconstructed particle had a null daughter, please report the bug." ); // something went wrong

  } while ( iMCP != mc_particles_linked_to_decay.end() && iPP != particles_in_decay.end() );

  return get_lowest_common_mother( mc_particles_to_compare );
}
//=============================================================================
bool BackgroundCategory::doAllFinalStateParticlesHaveACommonMother(
    const MCParticleVector& mc_mothers_final, const MCParticleVector& mc_particles_linked_to_decay,
    const ParticleVector& particles_in_decay )
...
// candidate Particle have a common MCParticle mother.
{
  ...
  if ( msgLevel( MSG::VERBOSE ) ) {
    ...
    for ( iP = mc_mothers_final.begin(); iP != mc_mothers_final.end(); ++iP ) {
      ...
      if ( *iP ) { verbose() << "PID of mother = " << ( *iP )->particleID().pid() << endmsg; }
    }
    ...
    for ( iPP = particles_in_decay.begin(); iPP != particles_in_decay.end(); ++iPP ) {
      ...
      if ( *iPP ) { verbose() << "PID of reconstructed particle = " << ( *iPP )->particleID().pid() << endmsg; }
    }
    ...
    for ( iMCP = mc_particles_linked_to_decay.begin(); iMCP != mc_particles_linked_to_decay.end(); ++iMCP ) {
      ...
      if ( *iMCP ) { verbose() << "PID of associated particle = " << ( *iMCP )->particleID().pid() << endmsg; }
    }
    verbose() << "####################################################################" << endmsg;
  }
  // End DEBUG
  ...
  do {
    ...
    if ( msgLevel( MSG::VERBOSE ) )
      verbose() << "Common mother is " << tempmother << " mother is " << ( *iP ) << " particle is " << ( *iPP )
  ...
  if ( carryon ) { // We have found a common mother!
    ...
    if ( !m_commonMother ) return false;
    // Exception( "Something has gone wrong when looking for the lowest common mother, please report the bug." );

    if ( msgLevel( MSG::VERBOSE ) ) verbose() << "Found common mother " << m_commonMother->particleID().pid() << endmsg;
...
// are any particles coming from said decay which we missed out in our reconstruction.
{
  ...
  for ( iP = mc_particles_linked_to_decay.begin(); iP != mc_particles_linked_to_decay.end(); ++iP ) {
    ...
    if ( ( *iP )->particleID().abspid() == 111 ) {
      ...
      mc_particles_linked_to_decay_without_pi0.insert( mc_particles_linked_to_decay_without_pi0.end(),
                                                       merged_pi0_daughters.begin(), merged_pi0_daughters.end() );
    } else
      mc_particles_linked_to_decay_without_pi0.push_back( *iP );
  }
  ...
  if ( msgLevel( MSG::VERBOSE ) ) {
    ...
    for ( iPP = finalstateproducts.begin(); iPP != finalstateproducts.end(); ++iPP ) {
      ...
      if ( *iPP ) { verbose() << "PID of daughter = " << ( *iPP )->particleID().pid() << endmsg; }
    }
    verbose() << "********************************************************************" << endmsg;
    verbose() << "**********************************************" << endmsg;
    verbose() << "Beginning printout of the associated particles" << endmsg;

    for ( iP = mc_particles_linked_to_decay_without_pi0.begin(); iP != mc_particles_linked_to_decay_without_pi0.end();
          ++iP ) {
      verbose() << "Associated particle = " << ( *iP ) << endmsg;
      if ( *iP ) { verbose() << "PID of associated particle = " << ( *iP )->particleID().pid() << endmsg; }
    }
    verbose() << "**********************************************" << endmsg;
  }
...
//=============================================================================
bool BackgroundCategory::areAllFinalStateParticlesCorrectlyIdentified(
    const ParticleVector& particles_in_decay, const MCParticleVector& mc_particles_linked_to_decay )
    ...
    if ( *iPmc && *iP ) {
      if ( isStable( ( *iP )->particleID().abspid() ) || ( *iP )->isBasicParticle() ) {
        ...
        if ( ( *iP )->particleID().abspid() != 22 ||
             m_calo2MC->isPureNeutralCaloFuture( *iP ) ) { // gamma->ee is considered as composite
...
// Checks whether the head of our decay chain has the "correct" PID.
{
  ...
  if ( !m_ppSvc->find( m_commonMother->particleID() ) ) {
    warning() << "The common mother has a particle ID," << m_commonMother->particleID()
              << "unknown to the particle service!" << endmsg;
    warning() << "I cannot tell if this is the particle you are looking for, so am classifying as 20, unless you are\
                  looking for an inclusive decay, in which case it will be classified 40/50. "
              << endmsg;
    return 0;
  }
...
// Check whether we missed any resonances in the decay chain.
{
  ...
  if ( topologycheck( candidate ) == topologycheck( m_commonMother ) )
    return false;
  else
    return true;
}
...
// for a particle of its PID.
{
  ...
  if ( !m_ppSvc->find( candidate->particleID() ) ) {
    warning() << "The reconstructed particle has a particle ID," << candidate->particleID()
              << "unknown to the particle service!" << endmsg;
    warning() << "I cannot tell its mass, so cannot tell if your background is low mass or not; as such I am "
                 "classifying it 40."
              << endmsg;
    return 0;
  }

  if ( ( m_commonMother->virtualMass() - m_lowMassCut ) < ( m_ppSvc->find( candidate->particleID() )->mass() ) )
    carryon = true;
...
// Checks if there are any ghosts in our final state Particles used to make the candidate Particle
{
  ...
  do {
    ...
    if ( ( *iP ) == 0 && ( isStable( ( *iPP )->particleID().abspid() ) || ( *iPP )->isBasicParticle() ) ) {
      carryon = false;
    ...
    if ( isStable( ( *iPP )->particleID().abspid() ) || ( *iPP )->isBasicParticle() ) {
      ...
      if ( *iP ) {
        ...
        if ( !gotacollision ) {
          ...
          gotacollision = true;
        } else if ( ( *iP )->primaryVertex() != tmpcollision )
          carryon = false;
      }
    }
    ++iP;
...
//=============================================================================
int BackgroundCategory::areAnyFinalStateParticlesFromAPrimaryVertex(
    const MCParticleVector& mc_particles_linked_to_decay ) {
        ...
        if ( *iVV ) {
          if ( ( *iVV )->products().empty() || isStable( ( *iPP )->particleID().abspid() ) ) {
            ...
            if ( ( *iPP )->originVertex()->isPrimary() ||
                 ( ( *iPP )->primaryVertex()->position() - ( *iPP )->originVertex()->position() ).Mag2() <
                     pow( m_rescut, 2 ) ) {
              ++howmanyfromPV;
            }
          }
...
// Ignores composites. The reconstructed mother is used to check the neutral association.
{
  ...
  for ( iP = particles_in_decay.begin(); iP != particles_in_decay.end(); ++iP ) {
    ...
    // ======  pure neutral calorimetric *basic* particle : gamma & pi0-merged (pi0-resolved is treated as a composite
    // particle)
    if ( ( *iP )->isBasicParticle() && m_calo2MC->isPureNeutralCaloFuture( *iP ) && !m_vetoN ) {
    ...
    // ====== other basic or stables particles (Warning : including gamma->ee as gamma is considered as stable)
      ...
      for ( mcPartIt = mcPartRange.begin(); mcPartIt != mcPartRange.end(); ++mcPartIt ) {
        ...
        if ( msgLevel( MSG::VERBOSE ) ) verbose() << "MCParticle is at " << mc_TempDeux << endmsg;
        if ( msgLevel( MSG::VERBOSE ) )
          verbose() << "MCParticle has PID = " << mc_TempDeux->particleID().pid() << endmsg;
        if ( msgLevel( MSG::VERBOSE ) ) verbose() << "MCParticle energy = " << mc_TempDeux->momentum().E() << endmsg;
        ...
        if ( associating_a_neutral &&
             ( ( reconstructed_mother->charge() != 0 &&
                 ( reconstructed_mother->particleID().pid() == mc_TempDeux->particleID().pid() ) ) ||
               ( reconstructed_mother->charge() == 0 &&
                 ( reconstructed_mother->particleID().abspid() == mc_TempDeux->particleID().abspid() ) ) ) ) {
      ...
      if ( mc_correctPID ) {
        ...
        associated_mcparts.push_back( NULL ); // ghost
  ...
  for ( iDAP = m_daughtersAndPartners.begin(); iDAP != m_daughtersAndPartners.end(); ++iDAP ) {
    ...
    if ( msgLevel( MSG::VERBOSE ) ) {
      ...
      if ( ( *iDAP ).second != NULL )
        verbose() << "Associated particle has PID " << ( ( *iDAP ).second )->particleID().pid() << endmsg;
      else
        verbose() << "There is no associated particle" << endmsg;
    }
  }
...
bool BackgroundCategory::foundClones( const MCParticleVector& mc_particles_linked_to_decay ) {
  ...
  for ( MCParticleVector::const_iterator iPP1 = mc_particles_linked_to_decay.begin();
    ...
    if ( !( *iPP1 ) )
      continue; // We already checked for ghosts any null associations
                // here are intermediates and we don't care about them
    for ( MCParticleVector::const_iterator iPP2 = ( iPP1 + 1 ); iPP2 != mc_particles_linked_to_decay.end(); ++iPP2 ) {
...
StatusCode BackgroundCategory::initialize() {
  ...
  if ( !msgLevel( MSG::VERBOSE ) ) m_printDecay.setEnabled( false );
...
DECLARE_COMPONENT( BackgroundCategory )
